...
export function GridDisplay({
  ...
  // Calculate overall width for the uniform grid (11 tiles per row, no offset)
  // Each tile contributes TILE_BASE_WIDTH / 2 to the span.
  // (11 tiles * 0.5W) + 0.5W for the last half-tile = 6 * W
  ...
  const getTilePosition = (r: number, c: number) => {
    // No horizontal offset for odd rows in this layout
  ...
  const handleDragStart = useCallback((event: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>, r: number, c: number) => {
    ...
    if (!gridDataRef.current[r]?.[c]) return; // Ensure a tile exists at the start point
  ...
  const handleDragMove = useCallback(async (event: MouseEvent | TouchEvent) => {
    ...
    if (event.type === 'touchmove' && event.cancelable) {
      // event.preventDefault(); // Temporarily commented out to see if it helps with some mobile scroll issues
    }
  ...
  const handleDragEnd = useCallback(() => {
    ...
    if (currentDragState && currentDragState.dragAxisLocked && currentDragState.draggedLineCoords && currentDragState.draggedLineCoords.length >= 1) {
      ...
      // if (numStepsRaw !== 0) { // Commit only occurs if steps > 0 in page.tsx
        ...
        // Defer commit to avoid React state update errors
        setTimeout(() => {
  ...
  useEffect(() => {
    if (activeDrag) {
      ...
      document.addEventListener('touchmove', handleDragMove, { passive: true }); // passive: true may help scrolling
  ...
  const getLineDisplacementVector = (
    ...
    axis: DragAxis 
      ...
      if (!lineCoords || lineCoords.length === 0 || !axis) return { dx: 0, dy: 0 };

      if (axis === 'row') {
          return { dx: visualTilesPerRow * (TILE_BASE_WIDTH / 2) + (TILE_BASE_WIDTH /2) , dy: 0 }; 
      } else { 
          const unitShiftDistance = TILE_BASE_WIDTH * 0.7; 
